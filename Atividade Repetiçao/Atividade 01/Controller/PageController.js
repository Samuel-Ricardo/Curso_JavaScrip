    ...
    if(check() === true){
        ...
        if(start < final){
           ...
           // for(start; start != final; start++){
             ...
             count =   increasing(start, final, interval)
            ...
            //for(start; start != final; start--){
             ...
             //        count = count + ` ${start} \u{1f449} `
              //  }
              count =   decreasing(start, final, interval)
            }
        ...
        result.innerText = `dados incompletos ;-;`
    }
}

function increasing(start, final, interval){

    var count = ""

    for(start; start != final; start += interval){
...
function decreasing(start, final, interval){
...
function check(){
    ...
    var finalInput = document.querySelector("input#finalNumber")
     ...
     else if(intervalInput.value.length == 0 || interval <= 0){
